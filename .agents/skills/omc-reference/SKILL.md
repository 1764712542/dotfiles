...
- `omc ask <Codex|codex|gemini|antigravity>`
- `/ccg`
...
### Team orchestration
- Codex 2.1.178+ uses one implicit agent team per session. Spawn teammates directly with Agent/Task using distinct `name` values; do not call removed `TeamCreate`/`TeamDelete` tools or rely on `team_name` for native routing.
